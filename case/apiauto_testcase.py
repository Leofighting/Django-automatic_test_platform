...
class ApiFlow(unittest.TestCase):
    ...
    def test_readSQLcase(self):
        sql = "select id, api_name, api_url, api_method, api_param_value, api_result, api_status " \
              "from apitest_apistep where apitest_apistep.api_test_id=2"
        coon = pymysql.connect(user="root", passwd="123qwe", db="automatic",
                               port=3306, host="127.0.0.1", charset="utf-8")
        cursor = coon.cursor()
        ...
        if method.upper() == "GET":
            ...
            if method.upper() == "POST":
                ...
                if "=" in urlParam(param):
                    ...
                    print(
                        str(case_id) + ' request is ' + new_url.encode('utf-8') + ' body is ' + urlParam(param).encode(
                            'utf - 8'))
...
def writeResult(case_id, result):
    ...
    sql = "UPDATE apitest_apistep " \
          "set apitest_apistep.apistatus=%s,apitest_apistep.create_time=%s " \
...
def caseWriteResult(case_id, result):
    result = result.encode('utf-8')
    now = time.strftime("%Y-%m-%d %H:%M:%S")
    sql = "UPDATE apitest_apitest " \
          "set apitest_apitest.apitestresult=%s,apitest_apitest.create_time=%s " \
...
def writeBug(bug_id, interface_name, request, response, res_check):
    ...
    print(bugdetail)
    sql = "insert into bug_bug (bug_name, bug_detail, bug_status, bug_level, " \
          "bug_creater, bug_assign, create_time, prodict_id) " \
          "values ('%s','%s','1','1','LEO', 'LEO', '%s', '2');" % (bugname, pymysql.escape_string(bugdetail), now)
    coon =pymysql.connect(user='root', passwd='test123456', db='autotest',
                          port=3306, host='127.0.0.1', charset='utf8')
    cursor = coon.cursor()
    cursor.execute(sql)
